...
type Router struct {
	...
	// Config is for application config.
	Config *Config
...
func New(ctx interface{}, cfg *Config) *Router {
...
func NewWithPrefix(ctx interface{}, cfg *Config, pathPrefix string) *Router {
	r := New(ctx, cfg)
...
// FIXME: copy-paste error in original source, comment is wrong
// NotFound sets the specified function as the not-found handler (when no route matches) and returns the router.
// Note that only the root router can have a NotFound handler.
...
func validateHandler(vfn reflect.Value, ctxType reflect.Type) {
	...
	if !isValidHandler(vfn, ctxType, reflect.TypeOf(resp).Out(0), reflect.TypeOf(req)) {
		panic(instructiveMessage(vfn, "a handler", "handler", "rw mojito.ResponseWriter, req *mojito.Request", ctxType))
	}
}
...
func validateErrorHandler(vfn reflect.Value, ctxType reflect.Type) {
	...
	if !isValidHandler(vfn, ctxType, reflect.TypeOf(resp).Out(0), reflect.TypeOf(req), emptyInterfaceType) {
		panic(instructiveMessage(vfn, "an error handler", "error handler", "rw mojito.ResponseWriter, req *mojito.Request, err interface{}", ctxType))
	}
}
...
func validateNotFoundHandler(vfn reflect.Value, ctxType reflect.Type) {
	...
	var resp func() ResponseWriter
	if !isValidHandler(vfn, ctxType, reflect.TypeOf(resp).Out(0), reflect.TypeOf(req)) {
		panic(instructiveMessage(vfn, "a 'not found' handler", "not found handler", "rw mojito.ResponseWriter, req *mojito.Request", ctxType))
	}
}
...
func validateOptionsHandler(vfn reflect.Value, ctxType reflect.Type) {
	...
	if !isValidHandler(vfn, ctxType, reflect.TypeOf(resp).Out(0), reflect.TypeOf(req), reflect.TypeOf(methods)) {
		panic(instructiveMessage(vfn, "an 'options' handler", "options handler", "rw mojito.ResponseWriter, req *mojito.Request, methods []string", ctxType))
	}
}
...
func validateMiddleware(vfn reflect.Value, ctxType reflect.Type) {
	...
	if !isValidHandler(vfn, ctxType, reflect.TypeOf(resp).Out(0), reflect.TypeOf(req), reflect.TypeOf(n)) {
		panic(instructiveMessage(vfn, "middleware", "middleware", "rw mojito.ResponseWriter, req *mojito.Request, next mojito.NextMiddlewareFunc", ctxType))
	}
}
...
//  - args is like "rw mojito.ResponseWriter, req *mojito.Request, next mojito.NextMiddlewareFunc"
