...
func TestMojitoRun(t *testing.T) {
	// just test that Run doesn't bomb
	go New(&Options{}).Run(":3000")
}

func TestMojitoServeHTTP(t *testing.T) {
	result := ""
	response := httptest.NewRecorder()

	m := New(&Options{})
	m.Use(HandlerFunc(func(rw http.ResponseWriter, r *http.Request, next http.HandlerFunc) {
		result += "foo"
		next(rw, r)
		result += "ban"
	}))
...
// Ensures that a Mojito middleware chain
